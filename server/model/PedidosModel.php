...
class Pedidos
{
	...
	{
		$precio = GMySqli::getRegister( 'Menus', array( 'precio' ), 'idMenu = '. $food[ 'id' ] );
		$precio = $precio[ 'precio' ];

		if( GMySqli::existRegister( 'PedidosMenus', 'Pedidos_nroMesa = ' . $this->mesa . ' AND Menus_idMenu= ' . $food['id'] ) )
		{
			$currentAmount = GMySqli::getRegister( 'PedidosMenus' , array( 'cantidad' ), 
				'Pedidos_nroMesa=' . $this->mesa . ' AND Menus_idMenu = ' . $food['id'] );

			$currentAmount = $currentAmount[ 'cantidad' ];
			$added = $food[ 'amount' ] - $currentAmount;

			GMySqli::updateRegister( 'PedidosMenus', 
										'cantidad = ' . ( $currentAmount + 1 ) . ', fecha = curdate(), hora = curtime()',
										'Pedidos_nroMesa = '. $this->mesa . ' AND Menus_idMenu = ' . $food['id'] );
		}				
		else
		{
			GMySqli::setRegister( 'PedidosMenus', 
				array( 'Pedidos_nroMesa', 'Menus_idMenu', 'cantidad', 'fecha', 'hora' ), 
	...
	{
		$precio = GMySqli::getRegister( 'Pizzas', array( 'precio' ), 'idPizza = '. $food [ 'id' ]  );
		$precio = $precio[ 'precio' ];

		if( GMySqli::existRegister( 'PedidosPizzas', 'Pedidos_nroMesa = ' . $this->mesa .  ' AND Pizzas_idPizza= ' . $food['id'] ) )
		{
			$currentAmount = GMySqli::getRegister( 'PedidosPizzas' , array( 'cantidad' ), 
				'Pedidos_nroMesa=' . $this->mesa . ' AND Pizzas_idPizza = ' . $food['id'] );

			$currentAmount = $currentAmount[ 'cantidad' ];

			GMySqli::updateRegister( 'PedidosPizzas', 'cantidad = ' . ( $currentAmount + 1 ) . ', fecha = curdate(), hora = curtime()', 
				'Pedidos_nroMesa = '. $this->mesa . ' AND Pizzas_idPizza = ' . $food['id'] );
		}
		...
		else
		{
			GMySqli::setRegister( 'PedidosPizzas',
				array( 'Pedidos_nroMesa', 'Pizzas_idPizza', 'cantidad', 'fecha', 'hora' ), 
				$this->mesa . ', ' . $food[ 'id' ] . ', ' . $food[ 'amount'] . ', curdate(), curtime()' );
		}
	...
	{
		$precio = GMySqli::getRegister( 'Bebidas', array( 'precio' ), 'idBebida = '. $food [ 'id' ]  );
		$precio = $precio[ 'precio' ];

		if( GMySqli::existRegister( 'PedidosBebidas', 'Pedidos_nroMesa = ' . $this->mesa .  ' AND Bebidas_idBebida= ' . $food['id'] ) )
		{
			$currentAmount = GMySqli::getRegister( 'PedidosBebidas' , array( 'cantidad' ), 
				'Pedidos_nroMesa=' . $this->mesa . ' AND Bebidas_idBebida = ' . $food['id'] );

			$currentAmount = $currentAmount[ 'cantidad' ];

			GMySqli::updateRegister( 'PedidosBebidas', 'cantidad = ' . ( $currentAmount + 1 ) . ', fecha = curdate(), hora = curtime()', 
				'Pedidos_nroMesa = '. $this->mesa . ' AND Bebidas_idBebida = ' . $food['id'] );
		}
		...
		else
		{
			GMySqli::setRegister( 'PedidosBebidas', 
				array( 'Pedidos_nroMesa', 'Bebidas_idBebida', 'cantidad', 'fecha', 'hora' ), 
				$this->mesa . ', ' . $food[ 'id' ] . ', ' . $food[ 'amount'] . ', curdate(), curtime()' );
		}
	...
	private function setMontoPedido()
	{
		...
		{
			$montoExis = GMySqli::getRegister( 'Pedidos', array( 'monto' ), 'nroMesa = ' . $this->mesa );
			...
			GMySqli::setRegister( 'Pedidos', array( 'nroMesa', 'monto' ), array( $this->mesa, $this->monto ) );
	}
	...
	public function getCartSelect( &$mesaElegida ) 
	{
		...
		$this->datos[ 'monto' ] = GMySqli::getRegister( 'Pedidos', array( 'monto' ), 'nroMesa = ' . $mesaElegida );
