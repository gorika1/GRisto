...
class Cocina
{
	...
	{
		if( $data[ 'type' ] == 1 )
			$delivered = GMySqli::getRegister( 'PedidosMenus', array( 'entregado' ),
			'Pedidos_nroMesa = ' . $data[ 'mesa' ] . ' AND Menus_idMenu = ' . $data[ 'id' ] );
		else
			$delivered = GMySqli::getRegister( 'PedidosPizzas', array( 'entregado' ),
	...
	private function getFoodById( $id, $type )
	{
		...
		{
			$name = GMySqli::getRegister( 'Menus', array( 'nombreMenu' ), 'idMenu = ' . $id );
			$name = $name[ 'nombreMenu' ];
		}
		else //if it needs the pizzas
		{
			$name = GMySqli::getRegister( 'Pizzas', array( 'nombrePizza' ), 'idPizza = ' . $id );
